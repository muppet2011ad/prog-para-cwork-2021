...
char **read_lines (FILE* src, int *count);

int main() {
    int num_lines;
    read_lines(stdin, &num_lines);
    return 0;
}
...
char **read_lines (FILE* src, int *count) {
    ...
    while (!feof(src)) { // Until we reach EOF
        ...
        if (num_lines >= lines_size) { // If we need to expand the array
            lines = realloc(lines, (lines_size+LINES_ARR_LEN)*sizeof(char*)); // Expand it
            if (lines == NULL) { // Complain and exit if realloc fails
                fprintf(stderr, "Error: failed to allocate memory when reading in string!");
                exit(1);
            }
            lines_size += LINES_ARR_LEN; // Update with new array size
        }
